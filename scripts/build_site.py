...
<body><a class="skip" href="#main">跳转到内容</a><header class="site-header"><a class="brand" href="index.html" aria-label="陈品元，返回首页">CP<span>Y.</span></a><span class="header-name">CHEN PINYUAN / 陈品元</span><nav aria-label="主导航"><a href="index.html#work">作品 <span>06</span></a><a href="index.html#about">关于</a><a href="mailto:1027997849@qq.com">联系 <span aria-hidden="true">↗</span></a></nav></header>
<main id="main">{content}</main>
<footer id="contact" class="container footer"><p class="eyebrow">LET’S MAKE SOMETHING PLAYABLE.</p><h2>一起创造值得探索的体验。</h2><a class="email" href="mailto:1027997849@qq.com">1027997849@qq.com <span aria-hidden="true">↗</span></a><div class="footer-bottom"><span>陈品元 · GAME & LEVEL DESIGN</span><a href="#main">回到顶部 ↑</a></div></footer>
...
def build_home():
    ...
    for i,p in enumerate(PROJECTS):
        rows+=f'<a class="work-row" href="{p["slug"]}.html"><div class="work-image"><img src="{A}{p["cover"]}" alt="{e(p["title"])} 游戏画面与作品设计" loading="lazy" decoding="async"><span class="work-number">0{i+1}</span></div><div class="work-copy"><p class="eyebrow">{p["tag"]}</p><h3>{p["title"]}</h3><p>{p["intro"]}</p><div class="work-meta"><span>{p["team"]}</span><span>{p["category"]}</span></div><span class="row-link">查看项目 <span aria-hidden="true">↗</span></span></div></a>'
    body=f'''<section class="container home-hero"><div class="hero-top"><p class="eyebrow">LEVEL DESIGN / PLAYER GUIDANCE / PUZZLES</p><span class="edition">PORTFOLIO / 2026</span></div><h1>DESIGN<br><span>FOR PLAY.</span></h1><div class="hero-bottom"><p class="intro-name">陈品元<span>关卡策划 / Level Designer</span></p><p>围绕玩法组织空间与挑战，<br>关注玩家如何找到路线、理解能力并完成解谜。</p><a class="text-link" href="#work">探索作品 ↓</a></div><a class="hero-feature" href="beneath-the-ice.html"><img src="{A}ice-cover.webp" alt="冰面之下：水上工厂的 3D 关卡白盒" fetchpriority="high"><span class="feature-label">SPATIAL STUDY / 3D BLOCKOUT</span><span class="feature-title">冰面之下 <span aria-hidden="true">↗</span></span></a></section>
...
def build_project(p,i):
    ...
    guide_links = ''.join(f'<a href="#section-{section}">{e(label)} <span aria-hidden="true">↓</span></a>' for label, section in highlights)
    ...
    if p['slug'] == '1000-action':
        media += f'''<section class="playable" aria-labelledby="playable-title"><p class="eyebrow">PLAY IN BROWSER / 网页试玩</p><h2 id="playable-title">直接试一关。</h2><p>建议使用电脑键鼠。点击后从 itch.io 加载游戏，可能需要等待；先点击游戏画面，再按空格运行或重试。</p><div class="game-stage" id="action-game" data-game-src="https://itch.io/embed-upload/16338738?color=0a0a0a"><button class="game-launch" type="button" aria-controls="action-game">加载并开始游戏</button></div><div class="game-controls"><button class="game-fullscreen" type="button" disabled>全屏游玩</button><button class="game-stop" type="button" hidden>关闭游戏</button><a class="text-link" href="https://zzoonng.itch.io/1000action" target="_blank" rel="noopener noreferrer">无法加载？前往 itch.io ↗</a></div><p class="game-status" role="status" aria-live="polite">尚未加载，不会自动播放声音。</p><noscript><p>当前浏览器未启用 JavaScript，请通过上方 itch.io 链接游玩。</p></noscript></section>'''
    media = gameplay + media + overview
    award=f'<p class="award">{p["award"]}</p>' if p.get('award') else ''
    links=''.join(f'<a class="text-link" href="{e(url)}" target="_blank" rel="noopener noreferrer">{e(label)} ↗</a>' for label,url in p['links'])
    content=f'''<div class="container"><section class="project-hero"><a class="back-link" href="index.html#work">← 全部作品</a><p class="eyebrow">0{i+1} / {p['tag']}</p><h1>{p['title']}</h1><p class="project-english">{p['en']}</p><p class="project-intro">{p['intro']}</p>{award}<dl class="project-facts"><div><dt>TYPE / 类型</dt><dd>{p['category']} · {p['team']}</dd></div><div><dt>PERIOD / 周期</dt><dd>{p['date']}</dd></div><div><dt>TOOLS / 工具</dt><dd>{p['engine']}</dd></div><div><dt>CONTRIBUTION / 职责</dt><dd>{p['role']}</dd></div></dl><div class="project-links">{links}</div>{media}</section><div class="case-layout"><aside class="case-nav" aria-label="项目内容导航"><p class="eyebrow">IN THIS PROJECT</p>{nav}</aside><div>{body}</div></div><a class="next-project" href="{nxt['slug']}.html"><span class="eyebrow">NEXT PROJECT</span><strong>{nxt['title']} <span aria-hidden="true">↗</span></strong></a></div>'''
